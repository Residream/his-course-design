...
void print_ward(Ward *w, int id_w, int name_w, int cap_w, int occ_w);             // 打印病房信息行
void print_ward_header(int id_w, int name_w, int cap_w, int occ_w);               // 打印病房表头
void print_ward_line(int id_w, int name_w, int cap_w, int occ_w);                 // 打印病房分隔线
void calc_ward_width(Ward *head, int *id_w, int *name_w, int *cap_w, int *occ_w); // 计算列宽

/*
 *病房功能函数
 */
Ward create_ward(const char *ward_id, const char *name, int capacity, int occupied); // 创建病房
