...
drugs_data = [
    ...
    # DR0004 高频处方用药，库存低 → 触发 ⚠ 补货
    ("DR0004","兰索拉唑肠溶片","达克普隆","兰索拉唑",40.00,15,"内科"),
    ...
    # DR0009 高频处方用药，库存中等 → 触发 ⚠ 关注
    ("DR0009","双氯芬酸钠缓释片","扶他林","止痛片",28.00,30,"外科"),
    ...
    # DR0028 高频处方用药，库存中等 → 触发 ⚠ 关注
    ("DR0028","硝酸甘油片","硝酸甘油","硝甘",12.50,25,"急诊科"),
    # DR0029 极低库存，稍加几条使用即可触发 ⚠ 补货
    ("DR0029","硫酸吗啡注射液","吗啡","吗啡",55.00,3,"急诊科"),
...
for did, _, _, _, _, _, dept in drugs_data:
    if dept in ("内科", "外科", "妇产科", "儿科", "通用"):
        pharmacy_drugs_lines.append(f"PH0001|{did}|500")
    if dept in ("急诊科", "通用"):
        pharmacy_drugs_lines.append(f"PH0002|{did}|200")
    pharmacy_drugs_lines.append(f"PH0003|{did}|300")
...
hosp_patient_pool = [
    ...
    # 额外妇产科住院（已出院，床位复用）→ 拉高妇产科住院需求占比，触发扩容建议
    ...
    # 再追加 5 条妇产科短期住院，彻底拉高需求占比跨过 10% 扩容阈值
    ...
    # 再追加 7 条儿科短期住院，彻底拉高需求占比跨过 10% 扩容阈值
...
# ===== 为 DR0029 吗啡 制造使用记录（触发库存预警演示）=====
# 从现有已完成看诊中挑出 5 条由急诊科医生接诊的，追加 DR0029 处方
...
for vid, reg_id, _vtime, v_status, _diag in visits:
    ...
    if reg and reg[2] in emergency_doctor_ids:
        ...
        morphine_added += 1
print(f"  为 DR0029 吗啡 注入 {morphine_added} 条处方（用于库存预警演示）")
